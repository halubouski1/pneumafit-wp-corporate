<?php
/**
 * Template for the ReLounge page (slug: relounge).
 */
get_header();
?>
  <section class="relounge-hero">
    <div class="relounge-hero__overlay"></div>

    <div class="container relounge-hero__container">


      <div class="relounge-hero__content">
        <div class="relounge-hero__bottom">
          <h1 class="relounge-hero__title">Gezieltes Rückentraining</h1>
          <div class="relounge-hero__cta">
            <button data-popup class="hero__btn hero__btn--primary relounge-hero__btn">
              Jetzt Termin sichern
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                <path d="M7.5 4.37512L10.625 7.50011L7.5 10.6251" stroke="currentColor" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"/>
                <path d="M0.625 0.625V4.99999C0.625 5.66303 0.888388 6.29891 1.35723 6.76774C1.82607 7.23658 2.46194 7.49997 3.12498 7.49997H10.6249" stroke="currentColor" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"/>
              </svg>
            </button>
            <span class="relounge-hero__subtitle">weil Ihr Wohlbefinden nicht warten sollte</span>
          </div>
        </div>
      </div>
    </div>
  </section>
  <section class="section-philosophy" data-aos="fade-up" data-aos-duration="900">

    <!-- 8 vertical lines, full height -->
    <div class="philosophy-lines" aria-hidden="true">
      <div class="philosophy-line l-1"></div>
      <div class="philosophy-line l-2"></div>
      <div class="philosophy-line l-3"></div>
      <div class="philosophy-line l-4"></div>
      <div class="philosophy-line l-5"></div>
      <div class="philosophy-line l-6"></div>
      <div class="philosophy-line l-7"></div>
      <div class="philosophy-line l-8"></div>
    </div>

    <!-- Main content: label left + content right -->
    <div class="philosophy-top">
      <div class="philosophy-wrapper">
        <span class="philosophy-label">
          Der Rücken im Fokus
        </span>
        <span class="philosophy-descr">Durch gezielte Anwendungen wie Massagen, Dehnungen und Stabilisierungsübungen werden muskuläre Dysbalancen ausgeglichen und die Haltung optimiert.</span>
      </div>
      <div class="philosophy-main">
        <h2 class="philosophy-title">reLounge ist eine Rückentherapie, die darauf abzielt, Rückenschmerzen und Verspannungen zu lindern sowie die Beweglichkeit und Flexibilität der Wirbelsäule zu verbessern.</h2>
        <p class="philosophy-text">Das ganzheitliche Konzept der ReLounge ermöglicht eine individuelle und effektive Behandlung, um langfristige Rückengesundheit und Schmerzfreiheit zu fördern.</p>
      </div>
    </div>

    <!-- Bottom words positioned between the lines -->
    <div class="philosophy-words" aria-hidden="true">
      <!-- col 1: 296px — 2 words centered as group -->
      <div class="pw-group">
        <span class="p-word p-word--1">Straffung</span>
        <span class="p-word p-word--2">Regeneration</span>
      </div>
      <div></div><!-- col 2: 178px spacer -->
      <!-- col 3: 296px — 1 word centered -->
      <span class="p-word p-word--3">Glättung</span>
      <div></div><!-- col 4: 178px spacer -->
      <!-- col 5: 296px — 2 words centered as group -->
      <div class="pw-group">
        <span class="p-word p-word--4">Konturierung</span>
        <span class="p-word p-word--5">Elastizität</span>
      </div>
      <div></div><!-- col 6: 178px spacer -->
      <div></div><!-- col 7: 296px empty (lines 7→8) -->
      <!-- col 8: remaining — 1 word centered -->
      <span class="p-word p-word--6">Verjüngung</span>
    </div>

  </section>

  <section class="section-reviews section-process" data-aos="fade-up" data-aos-duration="900">
    <div class="container section-reviews__container section-process__container">
      <div class="section-reviews__inner">

        <div class="section-process__top">
          <div class="section-process__left">
            <h2 class="section-title section-reviews__title section-process__title">Ihre Vorteile bei der reLounge Behandlung</h2>
          </div>
        </div>

        <div class="section-reviews__bottom">
          <div class="section-reviews__nav">
            <button class="reviews-btn reviews-btn--prev" id="reviewsPrev">
              <svg width="13" height="13" viewBox="0 0 13 13" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
              <mask id="mask0_264_650" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="13" height="13">
              <rect width="13" height="13" transform="matrix(-1 0 0 1 13 0)" fill="url(#pattern0_264_650)"/>
              </mask>
              <g mask="url(#mask0_264_650)">
              <ellipse cx="9.39453" cy="6.14453" rx="9.39453" ry="6.14453" transform="matrix(-1 0 0 1 16.9609 0.711914)"/>
              </g>
              <defs>
              <pattern id="pattern0_264_650" patternContentUnits="objectBoundingBox" width="1" height="1">
              <use xlink:href="#image0_264_650" transform="scale(0.00195312)"/>
              </pattern>
              <image id="image0_264_650" width="512" height="512" preserveAspectRatio="none" xlink:href="data:image/png;base64,"/>
              </defs>
              </svg>
            </button>
            <button class="reviews-btn reviews-btn--next" id="reviewsNext">
              <svg width="13" height="13" viewBox="0 0 13 13" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
              <mask id="mask0_264_647" style="mask-type:alpha" maskUnits="userSpaceOnUse" x="0" y="0" width="13" height="13">
              <rect width="13" height="13" transform="matrix(1 0 0 -1 0 13)" fill="url(#pattern0_264_647)"/>
              </mask>
              <g mask="url(#mask0_264_647)">
              <ellipse cx="9.39453" cy="6.14453" rx="9.39453" ry="6.14453" transform="matrix(1 0 0 -1 -3.96094 12.2881)"/>
              </g>
              <defs>
              <pattern id="pattern0_264_647" patternContentUnits="objectBoundingBox" width="1" height="1">
              <use xlink:href="#image0_264_647" transform="scale(0.00195312)"/>
              </pattern>
              <image id="image0_264_647" width="512" height="512" preserveAspectRatio="none" xlink:href="data:image/png;base64,"/>
              </defs>
              </svg>
            </button>
          </div>
  
          <div class="section-reviews__slider-wrap">
            <div class="reviews-clip-left">
            <div class="swiper reviews-swiper">
              <div class="swiper-wrapper">
  
                <div class="swiper-slide">
                  <div class="review-card">
                    <div class="review-card__inner">
                      <div class="review-card__header">
                        <svg width="14" height="19" viewBox="0 0 14 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M9.32 0.499961C9.32 0.299561 9.2003 0.118461 9.0159 0.0399612C8.8314 -0.0385389 8.6179 0.000660986 8.4734 0.139561L6.6955 1.84916C4.0483 4.39456 1.8114 7.33476 0.0646 10.5651C0.0235 10.6378 0 10.7217 0 10.8112C0 11.0873 0.2239 11.3112 0.5 11.3112H4.81V18.5C4.81 18.698 4.9268 18.8774 5.108 18.9574C5.2891 19.0374 5.5003 19.003 5.6467 18.8696L6.4422 18.145C9.2269 15.6083 11.5902 12.6446 13.4432 9.36496L13.9353 8.49416C14.0228 8.33936 14.0215 8.14976 13.9318 7.99616C13.8422 7.84256 13.6778 7.74816 13.5 7.74816H9.32V0.499961Z" fill="#4D6EFF" />
                        </svg>
                      </div>
                      <div class="review-card__bottom">
                        <h3 class="review-card__service">Ausgleich von muskulären Dysbalancen</h3>
                        <p class="review-card__text">Muskelungleichgewichte werden gezielt ausgeglichen. Dies fördert eine bessere Beweglichkeit und Stabilität.</p>
                      </div>
                    </div>
                  </div>
                </div>
  
                <div class="swiper-slide">
                  <div class="review-card">
                    <div class="review-card__inner">
                      <div class="review-card__header">
                        <svg width="20" height="15" viewBox="0 0 20 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M5.15 0C2.3691 0 0 2.0793 0 4.7798C0 6.6427 0.8723 8.2112 2.0285 9.5104C3.1807 10.8052 4.6671 11.892 6.011 12.8016L8.3296 14.3711C8.5835 14.543 8.9165 14.543 9.1704 14.3711L11.489 12.8016C11.9187 12.5108 12.363 12.2018 12.8063 11.8737C12.9223 11.7879 12.9592 11.6312 12.8952 11.5019L11.5324 8.7508C11.468 8.6209 11.2911 8.6005 11.1989 8.7123L11.1261 8.8005C10.7461 9.2611 10.1803 9.5278 9.5833 9.5278H7.75C6.6454 9.5278 5.75 8.6323 5.75 7.5278C5.75 6.4232 6.6454 5.5278 7.75 5.5278H8.452C8.5714 5.5278 8.6846 5.4744 8.7605 5.3823L10.3323 3.4772C10.7706 2.946 11.4514 2.6776 12.1343 2.7669C12.8172 2.8562 13.4062 3.2906 13.6931 3.9167L14.4958 5.6681C14.5801 5.8518 14.8195 5.9008 14.9941 5.799C15.2913 5.6259 15.6358 5.5278 16 5.5278H17.1884C17.3394 5.5278 17.4677 5.4156 17.4801 5.2651C17.4932 5.1056 17.5 4.9438 17.5 4.7798C17.5 2.0793 15.1309 0 12.35 0C10.9165 0 9.6552 0.6721 8.75 1.5418C7.8448 0.6721 6.5835 0 5.15 0Z" fill="#4D6EFF" />
                          <path d="M12.5568 4.43754C12.4492 4.20274 12.2283 4.03984 11.9722 4.00634C11.7161 3.97284 11.4608 4.07354 11.2965 4.27274L9.2298 6.77784H7.75C7.3358 6.77784 7 7.11364 7 7.52784C7 7.94204 7.3358 8.27784 7.75 8.27784H9.5833C9.8072 8.27784 10.0194 8.17784 10.1619 8.00514L11.6919 6.15054L13.4849 10.0625C13.5925 10.2973 13.8133 10.4602 14.0694 10.4937C14.3255 10.5272 14.5808 10.4265 14.7452 10.2273L16.3535 8.27784H18.75C19.1642 8.27784 19.5 7.94204 19.5 7.52784C19.5 7.11364 19.1642 6.77784 18.75 6.77784H16C15.7761 6.77784 15.5639 6.87784 15.4215 7.05054L14.3498 8.34954L12.5568 4.43754Z" fill="#4D6EFF" />
                        </svg>
                      </div>
                      <div class="review-card__bottom">
                        <h3 class="review-card__service">Verbesserung der Körperhaltung</h3>
                        <p class="review-card__text">Die Haltung wird nachhaltig optimiert. Der Körper richtet sich auf und wirkt entspannter und ausbalancierter.</p>
                      </div>
                    </div>
                  </div>
                </div>
  
                <div class="swiper-slide">
                  <div class="review-card">
                    <div class="review-card__inner">
                      <div class="review-card__header">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M12.0625 2.06508C11.7729 1.9381 11.4876 2.02693 11.3041 2.21382C11.1259 2.39536 11.0367 2.67243 11.0992 2.95179C11.1979 3.39273 11.25 3.85148 11.25 4.3227C11.25 5.39854 10.9789 6.15205 10.5123 6.7767C10.038 7.41168 9.35149 7.92863 8.48466 8.51192C8.47103 8.52109 8.45834 8.53158 8.44678 8.54324L8.34179 8.64903C6.48195 9.85058 5.25 11.9427 5.25 14.3227C5.25 18.0506 8.27208 21.0727 12 21.0727C15.7279 21.0727 18.75 18.0506 18.75 14.3227C18.75 13.2281 18.4893 12.1938 18.0264 11.2789C17.9887 11.2045 17.9164 11.1538 17.8336 11.1437C17.7508 11.1335 17.6684 11.1654 17.614 11.2286C17.5435 11.3103 17.4744 11.391 17.4064 11.4705C16.5281 12.4967 15.8193 13.3248 14.3662 13.6947C14.3044 13.7105 14.266 13.6988 14.2387 13.6817C14.2067 13.6617 14.1716 13.6217 14.1476 13.5579C14.0975 13.4247 14.119 13.255 14.2216 13.1437C14.9988 12.3011 15.5509 11.1573 15.7482 9.53017C16.1177 6.48227 14.7325 3.23568 12.0625 2.06508Z" fill="#4D6EFF" />
                        </svg>
                      </div>
                      <div class="review-card__bottom">
                        <h3 class="review-card__service">Förderung der Rückengesundheit</h3>
                        <p class="review-card__text">Die Körperkonturen werden gezielt modelliert. Die Silhouette erscheint definierter und harmonischer.</p>
                      </div>
                    </div>
                  </div>
                </div>
  
                <div class="swiper-slide">
                  <div class="review-card">
                    <div class="review-card__inner">
                      <div class="review-card__header">
                        <svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                          <path d="M11.4792 1.1875C10.6035 1.1875 9.89583 1.89514 9.89583 2.77083V6.72917C9.89583 7.60486 10.6035 8.3125 11.4792 8.3125C12.3549 8.3125 13.0625 7.60486 13.0625 6.72917V2.77083C13.0625 1.89514 12.3549 1.1875 11.4792 1.1875Z" fill="#4D6EFF" />
                          <path d="M16.2292 5.14583C15.3535 5.14583 14.6458 5.85347 14.6458 6.72917V7.91667C14.6458 8.13528 14.8231 8.3125 15.0417 8.3125H16.2292C17.1049 8.3125 17.8125 7.60486 17.8125 6.72917C17.8125 5.85347 17.1049 5.14583 16.2292 5.14583Z" fill="#4D6EFF" />
                          <path d="M7.52083 10.6875C6.64514 10.6875 5.9375 11.3951 5.9375 12.2708V16.2292C5.9375 17.1049 6.64514 17.8125 7.52083 17.8125C8.39653 17.8125 9.10417 17.1049 9.10417 16.2292V12.2708C9.10417 11.3951 8.39653 10.6875 7.52083 10.6875Z" fill="#4D6EFF" />
                          <path d="M2.77083 10.6875C1.89514 10.6875 1.1875 11.3951 1.1875 12.2708C1.1875 13.1465 1.89514 13.8542 2.77083 13.8542C3.64653 13.8542 4.35417 13.1465 4.35417 12.2708V11.0833C4.35417 10.8647 4.17695 10.6875 3.95833 10.6875H2.77083Z" fill="#4D6EFF" />
                          <path d="M12.2708 9.89583C11.3951 9.89583 10.6875 10.6035 10.6875 11.4792C10.6875 12.3549 11.3951 13.0625 12.2708 13.0625H16.2292C17.1049 13.0625 17.8125 12.3549 17.8125 11.4792C17.8125 10.6035 17.1049 9.89583 16.2292 9.89583H12.2708Z" fill="#4D6EFF" />
                          <path d="M11.0833 14.6458C10.8647 14.6458 10.6875 14.8231 10.6875 15.0417V16.2292C10.6875 17.1049 11.3951 17.8125 12.2708 17.8125C13.1465 17.8125 13.8542 17.1049 13.8542 16.2292C13.8542 15.3535 13.1465 14.6458 12.2708 14.6458H11.0833Z" fill="#4D6EFF" />
                          <path d="M2.77083 5.9375C1.89514 5.9375 1.1875 6.64514 1.1875 7.52083C1.1875 8.39653 1.89514 9.10417 2.77083 9.10417H6.72917C7.60486 9.10417 8.3125 8.39653 8.3125 7.52083C8.3125 6.64514 7.60486 5.9375 6.72917 5.9375H2.77083Z" fill="#4D6EFF" />
                          <path d="M6.72917 1.1875C5.85347 1.1875 5.14583 1.89514 5.14583 2.77083C5.14583 3.64653 5.85347 4.35417 6.72917 4.35417H7.91667C8.13528 4.35417 8.3125 4.17695 8.3125 3.95833V2.77083C8.3125 1.89514 7.60486 1.1875 6.72917 1.1875Z" fill="#4D6EFF" />
                        </svg>
                      </div>
                      <div class="review-card__bottom">
                        <h3 class="review-card__service">Gefühl:</h3>
                        <p class="review-card__text">Die Behandlung ist völlig schmerzfrei und fühlt sich an wie eine wohltuende, warme Tiefengewebsmassage.</p>
                      </div>
                    </div>
                  </div>
                </div>
  
              </div>
            </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>

  <section class="section-for-back" data-aos="fade-up" data-aos-duration="900">
    <div class="for-back__inner">
      <div class="for-back__top">
        <div class="for-back__top-left">
          <h2 class="for-back__title">Für den Rücken - und gegen die Schmerzen</h2>
          <p class="for-back__desc">reLounge kombiniert die bewährtesten Methoden gegen Rückenschmerzen in einem System und hilft unter anderem bei:</p>
        </div>
      </div>

      <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/for-back.jpg" class="for-back__mobile-img" alt="" aria-hidden="true">

      <div class="for-back__cards">
        <div class="for-back__card for-back__card--1">
          <span class="for-back__card-title">Rückenschmerzen</span>
        </div>
        <div class="for-back__card for-back__card--2">
          <span class="for-back__card-title">Blockaden</span>
        </div>
        <div class="for-back__card for-back__card--3">
          <span class="for-back__card-title">Verkrampfungen</span>
        </div>
        <div class="for-back__card for-back__card--4">
          <span class="for-back__card-title">Schmerzattacken</span>
        </div>
        <div class="for-back__card for-back__card--5">
          <span class="for-back__card-title">Entzündungen</span>
        </div>
      </div>

      <p class="for-back__note">Unsere Physiopraxis in Leinfelden-Echterdingen ist immer für Sie da und freut sich auf Ihren Besuch.</p>
    </div>
  </section>
  <section class="section-therapies" data-aos="fade-up" data-aos-duration="900">
    <div class="therapies__desktop-layout">
    <div class="therapies__body">

      <div class="therapies__left-area">

        <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/relounge-bg-card-1.svg" class="therapy-bg therapy-bg--active" data-idx="0" aria-hidden="true">
        <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/relounge-bg-card-2.svg" class="therapy-bg" data-idx="1" aria-hidden="true">
        <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/relounge-bg-card-3.svg" class="therapy-bg" data-idx="2" aria-hidden="true">

        <div class="therapies__tabs">
          <button class="therapy-tab therapy-tab--active" data-index="0">[ Das Prinzip ]</button>
          <button class="therapy-tab" data-index="1">[ Fit im Alltag ]</button>
          <button class="therapy-tab" data-index="2">[ Der Behandlungsablauf ]</button>
        </div>

        <div class="therapies__card-content therapy-content--active" data-idx="0">
          <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/services-cards.png" class="therapy-card__swirls" aria-hidden="true">
          <div class="therapy-card__body">
            <div class="therapy-card__body--wrapper">
              <h3 class="therapy-card__title">Das Prinzip</h3>
              <p class="therapy-card__text">Das Prinzip von reLounge beruht auf einer innovativen Kombination aus bewährten Methoden und modernsten Technologien für ein optimiertes Rückentraining und die Behandlung von Rückenschmerzen.</p>
            </div>
          </div>
        </div>

        <div class="therapies__card-content" data-idx="1">
          <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/services-cards.png" class="therapy-card__swirls" aria-hidden="true">
          <div class="therapy-card__body">
            <div class="therapy-card__body--wrapper">
              <h3 class="therapy-card__title">Fit im Alltag</h3>
              <p class="therapy-card__text">Rückenschmerzen gelten als Volkskrankheit und stellen viele Menschen vor beträchtliche Herausforderungen im Alltag. Hier kommt reLounge ins Spiel – eine Spezialpraxis, die dafür entwickelt wurde, um genau diese Beschwerden gezielt und effizient anzugehen.</p>
            </div>
          </div>
        </div>

        <div class="therapies__card-content" data-idx="2">
          <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/services-cards.png" class="therapy-card__swirls" aria-hidden="true">
          <div class="therapy-card__body">
            <div class="therapy-card__body--wrapper">
              <h3 class="therapy-card__title">Der Behandlungsablauf</h3>
              <p class="therapy-card__text">Bei PneumaFit liegt der Fokus auf einer gezielten und effektiven Behandlung von Rückenschmerzen. Jeder Patient wird mit größter Sorgfalt betreut, wobei ein individuell zugeschnittener Behandlungsplan erstellt wird. Dieser beginnt mit einer umfassenden Anamnese, bei der die Daten und die Vorgeschichte der Patienten sorgfältig erfasst werden.</p>
            </div>
          </div>
        </div>

      </div>

      <!-- RIGHT: title/nav aligned with ear zone + image-only swiper -->
      <div class="therapies__right-area">

        <div class="therapies__title-nav">
          <div class="therapies__arrows">
            <button class="therapies-btn" id="therapiesPrev">
              <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/mask-group.svg" alt="Zurück" width="13" height="13">
            </button>
            <button class="therapies-btn" id="therapiesNext">
              <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/mask-group.svg" alt="Weiter" width="13" height="13" class="reviews-btn__icon--flip">
            </button>
          </div>
        </div>

        <div class="swiper therapies-swiper">
          <div class="swiper-wrapper">

            <div class="swiper-slide">
              <div class="therapy-card__right therapy-card__right--first therapy-card__right--overlay">
                <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/relounge-card.png" class="therapy-card__img" alt="Das Prinzip">
                <div class="therapy-overlay-card therapy-overlay-card--tr">
                  <span class="therapy-overlay-card__tag">[ Das Prinzip ]</span>
                  <p class="therapy-overlay-card__text">Im reLounge-Konzept werden individuelle Bedürfnisse berücksichtigt und das Training ist darauf ausgerichtet, spezifische Rückenprobleme gezielt anzugehen.</p>
                  <p class="therapy-overlay-card__text">Durch diese personalisierte Herangehensweise und die Synthese verschiedener Methoden, gewährleistet reLounge nicht nur eine Linderung bestehender Beschwerden sondern fördert auch präventiv die Rückengesundheit.</p>
                </div>
                <div class="therapy-overlay-card therapy-overlay-card--bl">
                  <span class="therapy-overlay-card__tag">[ Das Prinzip ]</span>
                  <p class="therapy-overlay-card__text">Im Kern des reLounge-Konzepts steht das Verständnis, dass ein gesunder Rücken nicht nur durch isolierte Übungen, sondern durch ein System aus ganzheitlichen Ansätzen erreicht wird. Dies inkludiert einerseits klassische physiotherapeutische Techniken, andererseits ergänzen wirksame EMS-Trainingsmethoden (Elektromyostimulation) das Spektrum. EMS trägt dazu bei, selbst tiefliegende Muskelschichten effektiv zu erreichen und zu stärken, was konventionelles Training oft nicht leistet.</p>
                </div>
              </div>
            </div>

            <div class="swiper-slide">
              <div class="therapy-card__right therapy-card__right--second therapy-card__right--overlay">
                <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/relounge-card.png" class="therapy-card__img" alt="Fit im Alltag">
                <div class="therapy-overlay-card therapy-overlay-card--tr">
                  <span class="therapy-overlay-card__tag">[ Fit im Alltag ]</span>
                  <p class="therapy-overlay-card__text">Die Kombination aus fachlicher Expertise und einem klaren Behandlungskonzept macht reLounge zu einer wertvollen Ressource für alle, die ihren Alltag wieder schmerzfrei bewältigen möchten.</p>
                </div>
                <div class="therapy-overlay-card therapy-overlay-card--bl">
                  <span class="therapy-overlay-card__tag">[ Fit im Alltag ]</span>
                  <p class="therapy-overlay-card__text">Mit einem ganzheitlichen System, das auf neuesten wissenschaftlichen Erkenntnissen basiert, vereinfacht reLounge den Weg zu einem gesunden und schmerzfreien Rücken. Mit seinem innovativen Ansatz hat sich PneumaFit als eine führende Adresse für alle etabliert, die nach einer effektiven und langanhaltenden Lösung für ihre Rückenschmerzen suchen.</p>
                </div>
              </div>
            </div>

            <div class="swiper-slide">
              <div class="therapy-card__right therapy-card__right--third therapy-card__right--overlay">
                <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/relounge-card.png" class="therapy-card__img" alt="Der Behandlungsablauf">
                <div class="therapy-overlay-card therapy-overlay-card--tr">
                  <span class="therapy-overlay-card__tag">[ Der Behandlungsablauf ]</span>
                  <p class="therapy-overlay-card__text">Während des gesamten Prozesses genießen die Patienten eine transparente und verständliche Kommunikation über alle Schritte der Behandlung. Zu den wichtigen Bestandteilen des Ablaufs gehören neben der physischen Therapie auch Beratung und Schulung zur Vorbeugung weiterer Beschwerden.</p>
                </div>
                <div class="therapy-overlay-card therapy-overlay-card--bl">
                  <span class="therapy-overlay-card__tag">[ Der Behandlungsablauf ]</span>
                  <p class="therapy-overlay-card__text">Die Nutzung modernster Technologien ermöglicht es uns, spezifische Problembereiche zu identifizieren und damit eine zielgerichtete Behandlung einzuleiten.</p>
                  <p class="therapy-overlay-card__text">Unsere Therapeuten nehmen sich Zeit, um einen ganzheitlichen Behandlungsansatz anzubieten, der nicht nur die Linderung von Rückenschmerzen verspricht, sondern auch darauf abzielt, langfristige Lösungen für Ihre Rückenprobleme zu finden.</p>
                </div>
              </div>
            </div>

          </div>
        </div>

      </div>

    </div>

    <div class="therapies-pagination"></div>
    </div><!-- /therapies__desktop-layout -->

    <div class="therapies__tablet-layout">

      <div class="therapies__tl-header">
        <h2 class="therapies__section-title">Unsere Therapieangebote für Sie</h2>
        <div class="therapies__arrows">
          <button class="therapies-btn" id="therapiesTLPrev">
            <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/mask-group.svg" alt="Zurück" width="13" height="13">
          </button>
          <button class="therapies-btn" id="therapiesTLNext">
            <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/mask-group.svg" alt="Weiter" width="13" height="13" class="reviews-btn__icon--flip">
          </button>
        </div>
      </div>

      <div class="therapies__tl-tabs">
        <button class="therapy-tl-tab therapy-tl-tab--active" data-index="0">Das Prinzip</button>
        <button class="therapy-tl-tab" data-index="1">Der Behandlungsablauf</button>
        <button class="therapy-tl-tab" data-index="2">Fit im Alltag</button>
      </div>

      <div class="therapies__tl-content">

        <div class="therapies__tl-panel therapies__tl-panel--active" data-idx="0">
          <div class="therapies__tl-panel-text">
            <h3 class="therapy-card__title">Das Prinzip</h3>
            <p class="therapy-card__text">Das Prinzip von reLounge beruht auf einer innovativen Kombination aus bewährten Methoden und modernsten Technologien für ein optimiertes Rückentraining und die Behandlung von Rückenschmerzen.</p>
          </div>
        </div>

        <div class="therapies__tl-panel" data-idx="2">
          <div class="therapies__tl-panel-text">
            <h3 class="therapy-card__title">Fit im Alltag</h3>
            <p class="therapy-card__text">Rückenschmerzen gelten als Volkskrankheit und stellen viele Menschen vor beträchtliche Herausforderungen im Alltag. Hier kommt reLounge ins Spiel – eine Spezialpraxis, die dafür entwickelt wurde, um genau diese Beschwerden gezielt und effizient anzugehen.</p>
          </div>
        </div>

        <div class="therapies__tl-panel" data-idx="1">
          <div class="therapies__tl-panel-text">
            <h3 class="therapy-card__title">Der Behandlungsablauf</h3>
            <p class="therapy-card__text">Bei PneumaFit liegt der Fokus auf einer gezielten und effektiven Behandlung von Rückenschmerzen. Jeder Patient wird mit größter Sorgfalt betreut, wobei ein individuell zugeschnittener Behandlungsplan erstellt wird. Dieser beginnt mit einer umfassenden Anamnese, bei der die Daten und die Vorgeschichte der Patienten sorgfältig erfasst werden.</p>
          </div>
        </div>

      </div>

      <div class="therapies__tl-pagination">
        <span class="tl-dot tl-dot--active" data-idx="0"></span>
        <span class="tl-dot" data-idx="1"></span>
        <span class="tl-dot" data-idx="2"></span>
      </div>

      <div class="therapies__tl-image">
        <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/relounge-card.png" class="therapies__tl-img therapies__tl-img--active" data-idx="0" alt="Das Prinzip">
        <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/relounge-card.png" class="therapies__tl-img" data-idx="1" alt="Fit im Alltag">
        <img src="<?php echo esc_url( get_template_directory_uri() ); ?>/assets/img/relounge-card.png" class="therapies__tl-img" data-idx="2" alt="Der Behandlungsablauf">
      </div>

      <div class="therapies__tl-chips therapies__tl-chips--active" data-idx="0">
        <div class="therapies__tl-chip">
          <span class="therapy-overlay-card__tag">[ Das Prinzip ]</span>
          <span class="therapy-overlay-card__text">
            Im reLounge-Konzept werden individuelle Bedürfnisse berücksichtigt und das Training ist darauf ausgerichtet, spezifische Rückenprobleme gezielt anzugehen.
            Durch diese personalisierte Herangehensweise und die Synthese verschiedener Methoden, gewährleistet reLounge nicht nur eine Linderung bestehender Beschwerden sondern fördert auch präventiv die Rückengesundheit.
          </span>
        </div>
        <div class="therapies__tl-chip">
          <span class="therapy-overlay-card__tag">[ Das Prinzip ]</span>
          <span class="therapy-overlay-card__text">
            Im Kern des reLounge-Konzepts steht das Verständnis, dass ein gesunder Rücken nicht nur durch isolierte Übungen, sondern durch ein System aus ganzheitlichen Ansätzen erreicht wird. Dies inkludiert einerseits klassische physiotherapeutische Techniken, andererseits ergänzen wirksame EMS-Trainingsmethoden (Elektromyostimulation) das Spektrum. EMS trägt dazu bei, selbst tiefliegende Muskelschichten effektiv zu erreichen und zu stärken, was konventionelles Training oft nicht leistet.
          </span>
        </div>
      </div>
      <div class="therapies__tl-chips" data-idx="1">
        <div class="therapies__tl-chip">
          <span class="therapy-overlay-card__tag">[ Fit im Alltag ]</span>
          <span class="therapy-overlay-card__text">
            Die Kombination aus fachlicher Expertise und einem klaren Behandlungskonzept macht reLounge zu einer wertvollen Ressource für alle, die ihren Alltag wieder schmerzfrei bewältigen möchten.
          </span>
        </div>
        <div class="therapies__tl-chip">
          <span class="therapy-overlay-card__tag">[ Fit im Alltag ]</span>
          <span class="therapy-overlay-card__text">
            Mit einem ganzheitlichen System, das auf neuesten wissenschaftlichen Erkenntnissen basiert, vereinfacht reLounge den Weg zu einem gesunden und schmerzfreien Rücken. Mit seinem innovativen Ansatz hat sich PneumaFit als eine führende Adresse für alle etabliert, die nach einer effektiven und langanhaltenden Lösung für ihre Rückenschmerzen suchen.
          </span>
        </div>
      </div>
      <div class="therapies__tl-chips" data-idx="2">
        <div class="therapies__tl-chip">
          <span class="therapy-overlay-card__tag">[ Der Behandlungsablauf ]</span>
          <span class="therapy-overlay-card__text">
            Während des gesamten Prozesses genießen die Patienten eine transparente und verständliche Kommunikation über alle Schritte der Behandlung. Zu den wichtigen Bestandteilen des Ablaufs gehören neben der physischen Therapie auch Beratung und Schulung zur Vorbeugung weiterer Beschwerden.
          </span>
        </div>
        <div class="therapies__tl-chip">
          <span class="therapy-overlay-card__tag">[ Der Behandlungsablauf ]</span>
          <span class="therapy-overlay-card__text">
            Die Nutzung modernster Technologien ermöglicht es uns, spezifische Problembereiche zu identifizieren und damit eine zielgerichtete Behandlung einzuleiten.
            Unsere Therapeuten nehmen sich Zeit, um einen ganzheitlichen Behandlungsansatz anzubieten, der nicht nur die Linderung von Rückenschmerzen verspricht, sondern auch darauf abzielt, langfristige Lösungen für Ihre Rückenprobleme zu finden.
          </span>
        </div>
      </div>

    </div><!-- /therapies__tablet-layout -->

  </section>

  <section class="section-rl-contact" data-aos="fade-up" data-aos-duration="900">
    <div class="rl-contact__inner">

      <p class="rl-contact__img-text">Heilpraktische Verfahren und T Shape2 gehören zur Komplementärmedizin/Ästhetik.</p>

      <div class="rl-contact__top">
        <span class="rl-contact__brand">PneumaFit</span>
        <div class="rl-contact__cards">
          <div class="rl-contact__card">
            <svg class="rl-contact__dot" width="7" height="7" viewBox="0 0 7 7" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><circle cx="3.5" cy="3.5" r="3.5" fill="white"/></svg>
            <span class="rl-contact__card-label">Adresse</span>
            <p class="rl-contact__card-text"><?php echo pneumafit_render_contact_items( pneumafit_footer_contact_items( 'adresse' ) ); ?></p>
          </div>
          <div class="rl-contact__card rl-contact__card--active">
            <svg class="rl-contact__dot" width="7" height="7" viewBox="0 0 7 7" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><circle cx="3.5" cy="3.5" r="3.5" fill="#363636"/></svg>
            <span class="rl-contact__card-label">Telefon</span>
            <p class="rl-contact__card-text"><?php echo pneumafit_render_contact_items( pneumafit_footer_contact_items( 'telefon' ) ); ?></p>
          </div>
          <div class="rl-contact__card">
            <svg class="rl-contact__dot" width="7" height="7" viewBox="0 0 7 7" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><circle cx="3.5" cy="3.5" r="3.5" fill="white"/></svg>
            <span class="rl-contact__card-label">E-mail</span>
            <p class="rl-contact__card-text"><?php echo pneumafit_render_contact_items( pneumafit_footer_contact_items( 'email' ) ); ?></p>
          </div>
          <div class="rl-contact__card">
            <svg class="rl-contact__dot" width="7" height="7" viewBox="0 0 7 7" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><circle cx="3.5" cy="3.5" r="3.5" fill="white"/></svg>
            <span class="rl-contact__card-label">Behandlungszeiten</span>
            <p class="rl-contact__card-text"><?php echo pneumafit_render_contact_items( pneumafit_footer_contact_items( 'behandlungszeiten' ) ); ?></p>
          </div>
        </div>
      </div>

      <div class="rl-contact__bottom">
        <h2 class="rl-contact__title">Kontakt</h2>
        <p class="rl-contact__text">Sie möchten einen Termin in unserer Praxis vereinbaren? Rufen Sie uns einfach an oder schreiben Sie uns eine E-Mail.</p>
        <button data-popup class="hero__btn hero__btn--primary rl-contact__btn">
          Termin vereinbaren
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            <path d="M7.5 4.37512L10.625 7.50011L7.5 10.6251" stroke="currentColor" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"/>
            <path d="M0.625 0.625V4.99999C0.625 5.66303 0.888388 6.29891 1.35723 6.76774C1.82607 7.23658 2.46194 7.49997 3.12498 7.49997H10.6249" stroke="currentColor" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
        </button>
      </div>

    </div>

    <div class="rl-contact__mobile">
      <h2 class="rl-contact__title">Kontakt</h2>
      <p class="rl-contact__text">Sie möchten einen Termin in unserer Praxis vereinbaren? Rufen Sie uns einfach an oder schreiben Sie uns eine E-Mail.</p>

      <?php
      $pneumafit_contact_form_id = (int) get_option( 'pneumafit_contact_form_id' );
      if ( $pneumafit_contact_form_id && function_exists( 'gravity_form' ) ) :
          gravity_form( $pneumafit_contact_form_id, false, false, false, null, true );
      else :
          ?>
      <form class="section-contact__form rl-contact__form">
        <input type="tel" class="section-contact__input" placeholder="+1 (000)-000-00-00">
        <button type="submit" class="section-contact__btn">
          Ich will diesen guten Muskeltonus
          <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            <path d="M7.5 4.375L10.625 7.49999L7.5 10.6249" stroke="#F1F1F1" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"/>
            <path d="M0.625 0.625V4.99999C0.625 5.66303 0.888388 6.29891 1.35723 6.76774C1.82607 7.23658 2.46194 7.49997 3.12498 7.49997H10.6249" stroke="#F1F1F1" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
        </button>
      </form>
      <?php endif; ?>
      <span class="section-contact__privacy">durch Klicken auf die Schaltfläche stimmen Sie der
        <?php
        $pneumafit_privacy = function_exists( 'get_field' ) ? get_field( 'privacy_link', 'option' ) : null;
        if ( is_array( $pneumafit_privacy ) && ! empty( $pneumafit_privacy['url'] ) ) :
            $pneumafit_privacy_text   = ! empty( $pneumafit_privacy['title'] ) ? $pneumafit_privacy['title'] : 'Datenschutzrichtlinie zu';
            $pneumafit_privacy_target = ! empty( $pneumafit_privacy['target'] ) ? ' target="' . esc_attr( $pneumafit_privacy['target'] ) . '" rel="noopener"' : '';
            ?>
          <a href="<?php echo esc_url( $pneumafit_privacy['url'] ); ?>"<?php echo $pneumafit_privacy_target; ?>><?php echo esc_html( $pneumafit_privacy_text ); ?></a>
        <?php else : ?>
          <a href="#">Datenschutzrichtlinie zu</a>
        <?php endif; ?>
      </span>

      <div class="rl-contact__mobile-cards">
        <div class="rl-contact__card">
          <svg class="rl-contact__dot" width="7" height="7" viewBox="0 0 7 7" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><circle cx="3.5" cy="3.5" r="3.5" fill="#363636"/></svg>
          <span class="rl-contact__card-label">Adresse</span>
          <p class="rl-contact__card-text"><?php echo pneumafit_render_contact_items( pneumafit_footer_contact_items( 'adresse' ) ); ?></p>
        </div>
        <div class="rl-contact__card rl-contact__card--active">
          <svg class="rl-contact__dot" width="7" height="7" viewBox="0 0 7 7" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><circle cx="3.5" cy="3.5" r="3.5" fill="#363636"/></svg>
          <span class="rl-contact__card-label">Telefon</span>
          <p class="rl-contact__card-text"><?php echo pneumafit_render_contact_items( pneumafit_footer_contact_items( 'telefon' ) ); ?></p>
        </div>
        <div class="rl-contact__card">
          <svg class="rl-contact__dot" width="7" height="7" viewBox="0 0 7 7" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><circle cx="3.5" cy="3.5" r="3.5" fill="#363636"/></svg>
          <span class="rl-contact__card-label">E-mail</span>
          <p class="rl-contact__card-text"><?php echo pneumafit_render_contact_items( pneumafit_footer_contact_items( 'email' ) ); ?></p>
        </div>
        <div class="rl-contact__card">
          <svg class="rl-contact__dot" width="7" height="7" viewBox="0 0 7 7" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><circle cx="3.5" cy="3.5" r="3.5" fill="#363636"/></svg>
          <span class="rl-contact__card-label">Behandlungszeiten</span>
          <p class="rl-contact__card-text"><?php echo pneumafit_render_contact_items( pneumafit_footer_contact_items( 'behandlungszeiten' ) ); ?></p>
        </div>
      </div>
    </div>
  </section>

<?php get_footer(); ?>
